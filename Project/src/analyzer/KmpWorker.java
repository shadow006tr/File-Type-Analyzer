...
public class KmpWorker implements Runnable {
    ...
    private void computeLPSArray(String pattern, int size, int[] lps) {
        int length = 0;                                                             // The length of the
                                                                                    // longest previous suffix
        int i = 1;                                                                  // Index in the array
        lps[0] = 0;                                                                 // The first element must be 0.

        while (i < size) {                                                          // running on the array
            if (pattern.charAt(i) == pattern.charAt(length)) {                      // if the prefix letter is equal
                                                                                    // to the suffix letter,
                length++;                                                           // we increment the length,
                lps[i] = length;                                                    // writing it to the current
                i++;                                                                // position in the array,
                                                                                    // and then moving on the array.

            } else {                                                                // If pattern[i] != pattern[length],
                if (length != 0) {                                                  // and the length isn't zero,
                    length = lps[length - 1];                                       // we are writing the previous
                                                                                    // length in the array.

                } else {                                                            // if the length is zero
                    lps[i] = length;                                                // we put zero in the current
                                                                                    // index in the array
                    i++;                                                            // and move on it.
                }
            }
